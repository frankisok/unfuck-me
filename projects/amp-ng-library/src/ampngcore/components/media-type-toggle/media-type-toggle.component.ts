...
@Component({
    ...
    templateUrl: './media-type-toggle.component.html',
    changeDetection: ChangeDetectionStrategy.OnPush,
    styleUrls: ['./media-type-toggle.component.scss'],
    standalone: false
})
export class MediaTypeToggleComponent extends UnSub implements OnInit {
  ...
  ngOnInit(): void {
      ...
      this.checkbox.nativeElement.checked = this.currentState.mediaType == this.VIDEO_STATE ? false : true

    if (MediaType) {
      this.dataOn = this.translate.instant('Video')
      this.dataOff = this.translate.instant('Audio')
    }
  }
  ...
  toggle() {
      ...
      this.eventBus.publish({
          ...
          eventPayload: {
              eventType: AMPEventType.TOGGLE_MEDIA_TYPE,
              eventData: this.currentState.mediaType
          }
      })
  ...
  getRelativePath(mediaType: MediaType) {
    ...
    const pathWithoutLastSegment = path.substring(0, lastSlashIndex);
    const _mediaType = mediaType === MediaType.AUDIO ? 'audio' : 'video'
    if (path === '/dam/library' || path === '/library') {
        return `/${path}/${_mediaType}`;
    }
    return `${pathWithoutLastSegment}/${_mediaType}`;
  }
